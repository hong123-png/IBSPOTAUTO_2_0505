...
log_file = os.path.join(os.path.dirname(os.path.abspath(__file__))+"\\log\\", 'stepC'+now+'.log')  # 日志文件路径
print('====================== stepC程序开始运行 ======================')
log_msg(log_file, "stepC程序开始运行", withLock=False)
...
if __name__ == '__main__':
    if not os.path.exists(upload_data_dir):
        ...
        log_msg(log_file, f"错误: 文件夹 {upload_data_dir} 不存在！", withLock=False)
        exit(1)

    lock_fileA = os.path.join(upload_data_dir, "LOCK3.txt")  # 构建完整的文件路径
    if os.path.exists(lock_fileA):
        log_msg(log_file, f"文件夹 {os.path.basename(os.path.normpath(upload_data_dir))} 中存在 LOCK3.txt 文件，程序退出！", withLock=False)
        exit(0)
    else:
        try:   
            # 创建文件
            with open(lock_fileA, "w") as f:
                ...
                log_msg(log_file, f"创建了目录 '{os.path.basename(os.path.normpath(upload_data_dir))}' 下的 LOCK3.txt 文件。", withLock=False)
            ...
            for file in os.listdir(upload_data_dir):
                ...
                if update_config_file(csv_file):
                    ...
                    file_path = os.path.join(upload_data_dir, csv_file)
                    ...
                    # csv_files_a1 = [f for f in os.listdir(upload_data_dir+"_a1") if f.endswith('.csv') and f.startswith('IBA_DONE3_')]
                    # if len(csv_files_a1) < len(csv_files):
                    #     new_file_path = os.path.join(upload_data_dir+"_a1", new_file_name)
                    # else:
            ...
            if os.path.exists(lock_fileA):
                ...
                print(f"成功删除了目录 '{upload_data_dir}' 下的 LOCK3.txt 文件。")
                log_msg(log_file, f"成功删除了目录 '{os.path.basename(os.path.normpath(upload_data_dir))}' 下的 LOCK3.txt 文件。", withLock=False)
                exit(0)

            log_msg(log_file, "stepC程序运行结束", withLock=False)
            print('====================== stepC程序运行结束 ======================')
            sys.exit(0)