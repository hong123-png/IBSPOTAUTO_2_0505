...
def Execute_Download_pics(count, try_count, asinDirs,pictures_path):
    ...
    for index, item in enumerate(getasins()):
        ...
        print(f'-----第{index + 2}行 数据的 ASIN 是：{item}')
        print(f'-----第{index + 2}行 数据的站点是：{siteType}')
        print(f'-----第{index + 2}行 数据的变体名称是：{variationName}')
        ...
        if asinDirs:
            ...
            if itemD in asinDirs:
                ...
                print(f'-----第{index + 2}行 数据的图片数量是：{file_count}')
                print(f'-----第{index + 2}行 数据的图片数量是：{len(get_pictures()[index].split(";"))}')
        ...
        if len(pics) != 0:
            ...
            print('testtestesetsetsetwset')
            if success_count == 0:
                try_count += 1
                print(folder_path)
                try:
                    shutil.rmtree(folder_path)  # 删除文件夹
                    print(f'-----第{index + 2}行 数据的图片下载失败，删除文件夹 {folder_path} -----')
                    # log_msg(log_file, f'第{index + 2}行 数据的图片下载失败，删除文件夹 {folder_path}', withLock=False)
                except Exception as e:
                    print(f'-----删除文件夹 {folder_path} 失败，错误信息：{e}')
                    # log_msg(log_file, f'删除文件夹 {folder_path} 失败，错误信息：{e}', withLock=False)
                    # try_count += 1
...
def rerun_counts(pictures_path):
    for i in range(5):
        ...
        if os.path.isdir(pictures_path):
            ...
            asinDirs = []
        e_result = Execute_Download_pics(count, try_count, asinDirs,pictures_path)
        print(f'-----第{i + 1}次执行结果：'+ str(e_result))
        if -1 == e_result:
            return False
        time.sleep(5)
    return True
            ...
            for file in os.listdir(upload_data_dir_processId):
                ...
                if update_config_file(csv_file):
                    ...
                    if not rerun_counts(pictures_path):
                        print(f"错误: 下载图片失败！::"+ pictures_path)
                        break
